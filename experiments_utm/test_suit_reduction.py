...
class TestSuiteCache:
    ...
    def in_cache_2(self, state, desired_velocity):
    ...
    def get_next_state_2(self, state, desired_velocity):
        roll = state[3]
        pitch = state[4] 
        yaw = state[5]
    ...
    #             state = point[:-1]
    #             ra = point[-1]
    #             if not selfin_cache_2.in_cache(state, ra):
    #                 # state_round = []
    #                 # body_state = self.body(state)
    #                 # key_state = body_state + state[12:18]
    ...
    #                     # next_state[0] = offset_body[0] - v_body[0] # * (next_state[18] - state[18])
    #                     # next_state[1] = offset_body[1] - v_body[1] # * (next_state[18] - state[18])
    #                     # next_state[2] = offset_body[2] - v_body[2] # * (next_state[18] - state[18])

    #                     next_state[3] -= roll 
    #                     next_state[4] -= pitch 
    #                     next_state[5] -= yaw

    #                     next_state[6] = v_body[0]
    #                     next_state[7] = v_body[1]
    #                     next_state[8] = v_body[2]

    #                     # next_state[6] = 0
    #                     # next_state[7] = 0
    #                     # next_state[8] = 0

    #                     # next_state[15] -= v_body[0]
    #                     # next_state[16] -=[self.__round(val, self.resolution[i]) for i, val in enumerate(desired_velocity)] v_body[1]
    #                     # next_state[17] -= v_body[2]
    ...
    #     Rroll = [[1,0,0],[0, np.cos(roll), np.sin(roll)],[0, -np.sin(roll), np.cos(roll)]]
    #     Rpitch = [[np.cos(pitch),0 , -np.sin(pitch)],[0,1,0],[np.sin(pitch),0 ,np.cos(pitch)]]
    #     Ryaw = [[np.cos(yaw), np.sin(yaw), 0],[-np.sin(yaw), np.cos(yaw), 0],[0,0,0]]
        
    #     vbody = np.array(Rroll)@np.array(Rpitch)@np.array(Ryaw)@np.array([vx, vy, vz])
    #     res = list(vbody) + state[9:12]
    #     return res
    ...
    #     # vbody = [vx, vy, vz]
    #     # vbody = list(np.array(Ryaw)@np.array([vx, vy, vz]))
    #     # vbody = list(np.array(Rroll)@np.array(Rpitch)@np.array(Ryaw)@np.array([vx, vy, vz]))
    #     vbody = self.convert_pos_to_body([vx, vy, vz], roll, pitch, yaw)

    #     res = [roll, pitch] + vbody

    #     vdesired_x = state[18] - vx 
    #     vdesired_y = state[19] - vy 
    #     vdesired_z = state[20] - vz  

    #     # vdesired_body = [vdesired_x, vdesired_y, vdesired_z]
    #     # vdesired_body = list(np.array(Ryaw)@np.array([vdesired_x, vdesired_y, vdesired_z]))
    #     # vdesired_body = list(np.array(Rroll)@np.array(Rpitch)@np.array(Ryaw)@np.array([vdesired_x, vdesired_y, vdesired_z]))
    #     vdesired_body = self.convert_pos_to_body([vdesired_x, vdesired_y, vdesired_z], roll, pitch, yaw)
        
    #     # key_state = res + state[12:18]
    #     # key_state = [roll, pitch] + state[12:15] + [state[15]-vx, state[16]-vy, state[17]-vz]
...
def generate_test_suite() -> List[Test]:
    ...
    for i, theta in enumerate(theta_list):
        for j, vint in enumerate(vint_list):
            for k, rho in enumerate(rho_list):
                ...
                test_case = Test(init=init, wp=waypoints_list, T=T_wp, idx=f"{0}_{j}_{k}", agent_list = ['drone0', 'drone1'])
...
def plot_trajectory(cache_trajectory, simulate_trajectory, idx = ''):
    color = ['b.','g.']
    x_cache_0 = []
    y_cache_0 = []
    x_cache_1 = []
...
def test_suite_reduction(TS: List[Test]):
    cache = TestSuiteCache()
    execs = []
    cache_hit = 0
    total_points = 0
    ...
    for k, test in enumerate(TS):
        ...
        for i in range(len(parsed_trajectory)-1):
            ...
            for j, agent_state in enumerate(alpha):
                ...
                if cache.in_cache(agent_state, ra):
                    next_state = cache.get_next_state(agent_state, ra)
                    # cache_hit += 1
                    trajectory_hit += 1
                    next_alpha.append(next_state)
                else:
...
def test_suite_reduction_2(TS: List[Test]):
    cache = TestSuiteCache()
    execs = []
    cache_hit = 0
    total_points = 0
...
def test_suite_reduction_3(TS: List[Test]):
    cache = TestSuiteCache()
    execs = []
    cache_hit = 0
    total_points = 0
        ...
        for i in range(len(parsed_trajectory)-1):
            ...
            for j, agent_state in enumerate(alpha):
                ...
                if cache.in_cache_2(agent_state, desired_velocity):
                    ...
                    not_in_cache = True
                    next_alpha = parsed_trajectory[i+1]
                    cache.add_test_run_2([parsed_trajectory[i], parsed_trajectory[i+1]])
...
if __name__ == "__main__":
    ...
    cache_hit, total_points, cache = test_suite_reduction_3(TS)
    cache_size = len(cache.cache_dict)
