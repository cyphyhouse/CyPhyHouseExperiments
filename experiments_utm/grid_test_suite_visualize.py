...
for i, theta in enumerate(theta_list):
    for j, vint in enumerate(vint_list):
        for k, rho in enumerate(rho_list):
            ...
            plt.plot(init_pos_0[0], init_pos_0[1],'b.')
            plt.plot([init_pos_0[0], init_pos_0[0]+init_lin_vel_0[0]*10], [init_pos_0[1], init_pos_0[1]+init_lin_vel_0[1]*10], 'b')

            plt.plot(init_pos_1[0], init_pos_1[1],'g.')
            plt.plot([init_pos_1[0], init_pos_1[0]+init_lin_vel_1[0]*10], [init_pos_1[1], init_pos_1[1]+init_lin_vel_1[1]*10], 'g')
            # waypoints_str = f"acas_grid_{i}_{j}_{k} = {waypoints_dict}"
            # print(waypoints_str)
            # with open(wp_fn, 'wb+') as f:
            #     pickle.dump([waypoints_dict, {'drone0', 'drone1'}], f)
            # run_simulation(f"{i}_{j}_{k}", yaml_fn, wp_fn)
